...
CREATE TABLE IF NOT EXISTS `iceberg_purge_job` (
  ...
  `lease_epoch`        BIGINT(20)     UNSIGNED NOT NULL DEFAULT 0 COMMENT 'monotonic fencing token incremented at every claim',
  `lease_expires_at`   BIGINT(20)     UNSIGNED DEFAULT NULL COMMENT 'server timestamp after which another worker may reclaim the batch',
  `heartbeat_at`       BIGINT(20)     UNSIGNED DEFAULT NULL COMMENT 'last successful lease heartbeat in milliseconds',
  ...
  `created_at`         BIGINT(20)     UNSIGNED NOT NULL COMMENT 'creation timestamp in milliseconds',
  `started_at`         BIGINT(20)     UNSIGNED DEFAULT NULL COMMENT 'first successful claim timestamp in milliseconds',
  `completed_at`       BIGINT(20)     UNSIGNED DEFAULT NULL COMMENT 'terminal timestamp in milliseconds',
  `updated_at`         BIGINT(20)     UNSIGNED NOT NULL COMMENT 'last durable job update in milliseconds',
  PRIMARY KEY (`purge_job_id`),
  KEY `idx_iceberg_purge_job_claim` (`state`, `lease_expires_at`, `updated_at`, `purge_job_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'durable bounded Iceberg purge batch headers';
...
CREATE TABLE IF NOT EXISTS `iceberg_purge_plan` (
  ...
  `target_count`      BIGINT(20)  UNSIGNED NOT NULL DEFAULT 0 COMMENT 'durable target count after the plan becomes READY',
  `root_target_id`    VARCHAR(64) DEFAULT NULL COMMENT 'root metadata target, which must have the greatest delete order',
  `created_at`        BIGINT(20)  UNSIGNED NOT NULL COMMENT 'plan creation timestamp in milliseconds',
  `completed_at`      BIGINT(20)  UNSIGNED DEFAULT NULL COMMENT 'timestamp when the exact target snapshot became READY',
  `updated_at`        BIGINT(20)  UNSIGNED NOT NULL COMMENT 'last durable plan update in milliseconds',
  PRIMARY KEY (`deletion_id`),
  KEY `idx_iceberg_purge_plan_job` (`purge_job_id`, `state`, `deletion_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'durable completeness marker for one deletion action physical target snapshot';
...
CREATE TABLE IF NOT EXISTS `iceberg_purge_target` (
  ...
  `target_uri`        MEDIUMTEXT     NOT NULL COMMENT 'exact object URI snapshotted before cleanup starts',
  ...
  `lease_epoch`       BIGINT(20)     UNSIGNED NOT NULL DEFAULT 0 COMMENT 'batch fencing epoch that claimed the target',
  ...
  `created_at`        BIGINT(20)     UNSIGNED NOT NULL COMMENT 'target snapshot timestamp in milliseconds',
  `updated_at`        BIGINT(20)     UNSIGNED NOT NULL COMMENT 'last progress update timestamp in milliseconds',
  PRIMARY KEY (`deletion_id`, `target_id`),
  KEY `idx_iceberg_purge_target_work` (`purge_job_id`, `deletion_id`, `state`, `delete_order`, `target_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'per-object progress ledger for restart-safe Iceberg hard deletion';
