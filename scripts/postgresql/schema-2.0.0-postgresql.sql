...
CREATE TABLE IF NOT EXISTS metalake_meta (
    ...
    deletion_id VARCHAR(64) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS entity_change_log (
    ...
    -- Operate type code: 1=ALTER, 2=DROP, 3=INSERT (reserved), 4=RESTORE. Codes are stable and never re-used.
    operate_type SMALLINT NOT NULL,
    created_at BIGINT NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_ecl_created_at ON entity_change_log(created_at);
COMMENT ON TABLE entity_change_log IS 'Append-only log of entity structural changes for targeted local cache invalidation';
...
COMMENT ON COLUMN entity_change_log.entity_full_name IS 'Dot-separated full name of the affected entity. For ALTER, stores the old name. For DROP and RESTORE, stores the entity name.';
COMMENT ON COLUMN entity_change_log.operate_type IS 'Operate type code: 1=ALTER, 2=DROP, 3=INSERT (reserved), 4=RESTORE. Codes are stable and never re-used.';
