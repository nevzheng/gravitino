...
public final class IcebergDeletionAuthorization {
  ...
  /**
   * Checks the current {@code DROP_TABLE} semantics against a live or retained table generation.
   *
   * <p>Normal authorization remains authoritative for live tables and parent owners. The narrow
   * fallback applies only when an exact deletion action is supplied: it verifies current parent-use
   * privileges and the owner relation stamped with that action's table ID and deletion ID.
   *
   * @param identifier full Gravitino table identifier
   * @param deletion exact retained action, or {@code null} for a live/missing table
   * @return whether the current principal may manage the deletion
   */
  public static boolean canDrop(NameIdentifier identifier, @Nullable EntityDeletionPO deletion) {
    ...
    return deletion != null
        ...
        && TableDeletionService.getInstance()
            .isRetainedOwner(deletion, PrincipalUtils.getCurrentPrincipal());
  }
}
