...
public class TestPolicyOperations extends BaseOperationsTest {
    ...
    resourceConfig.register(
        new AbstractBinder() {
          ...
          protected void configure() {
            ...
            bind(recoverableDeletionManager).to(RecoverableDeletionManager.class);
  ...
  @Test
  public void testDeletedPolicyReadAndRestore() {
    String etag =
    ...
    Assertions.assertArrayEquals(
        new DeletedEntityDTO[] {deletedPolicy},
    ...
    Assertions.assertEquals(
        deletedPolicy, itemResponse.readEntity(DeletedEntityResponse.class).getDeletedEntity());
    ...
    Response restore = patchRestorePolicy("984273", '\"' + etag + '\"');
    Assertions.assertEquals(Response.Status.OK.getStatusCode(), restore.getStatus());
    Assertions.assertEquals("policy1", restore.readEntity(PolicyResponse.class).getPolicy().name());
  ...
  public void testPolicyPatchMediaTypesCannotCrossMutate() {
    Response setResponse =
        target(policyPath(metalake))
            ...
            .method(
                "PATCH",
                Entity.entity(new PolicySetRequest(true), MediaType.APPLICATION_JSON_TYPE));
  ...
  public void testRestoreDeletedPolicyMapsRecoveryFailures() {
    ...
    Assertions.assertEquals(
        ErrorConstants.RECOVERY_CONFLICT_CODE, conflict.readEntity(ErrorResponse.class).getCode());
    verifyNoInteractions(policyManager);
  }
