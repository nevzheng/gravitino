...
- Headings are noun phrases that label content, not imperatives that command the reader. Drop filler verbs like "Use" when the noun phrase carries the meaning on its own ("Use Jupyter Notebook" -> "Jupyter Notebook", "Use `simple` Authentication" -> "`simple` Authentication"). When a verb is doing real work, replace "Use [tool] to [verb]" with "[verb] ... with [tool]" ("Use Spark to Access the Fileset" -> "Access the Fileset with Spark"). Two exceptions: CLI command references in operation lists keep imperative verbs to mirror command structure, and SQL/DDL section headings may mirror a SQL keyword verbatim (e.g., "Use the Catalog" mirroring `USE CATALOG`).
...
- Colons in headings earn their place only when they disambiguate two distinct technical concepts joined into one name: "Trino Connector: Hive Catalog" (the connector and the catalog it serves), "Spark Connector: User-Defined Functions" (the connector and the UDF subsystem). When the part after the colon is just a noun that could be appended directly, drop the colon: "Access: Users" reads as cleanly as "Access Users" without the decorative punctuation. Category-type prefixes like "Quick Start:" or "Tutorial:" are not a legitimate exception. They read as outlier styling when surrounding headings don't follow the same convention, and the action verb in a procedural heading ("Enable", "Configure", "Install") already signals the section's intent without a label.
