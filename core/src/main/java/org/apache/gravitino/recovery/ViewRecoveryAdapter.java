...
/** Adapts relational view metadata to the shared recoverable-deletion protocol. */
final class ViewRecoveryAdapter implements RecoverableEntityAdapter<ViewEntity> {

  private static final Logger LOG = LoggerFactory.getLogger(ViewRecoveryAdapter.class);

  @Nullable private final EntityCache entityCache;

  ViewRecoveryAdapter(@Nullable EntityCache entityCache) {
  ...
  public RecoveryEntityType recoveryType() {
    return RecoveryEntityType.VIEW;
  }

  @Override
  public List<RecoveryMetadata.DeletedSnapshot> listDeleted(Namespace namespace) {
    return ViewMetaService.getInstance().listDeletedViewsByNamespace(namespace).stream()
        .map(ViewRecoveryAdapter::deletedSnapshot)
  ...
  public List<RecoveryMetadata.LiveIdentity> listLiveInParent(Namespace namespace, long parentId) {
    return ViewMetaService.getInstance().listLiveViewPOsByNamespace(namespace).stream()
        .map(ViewRecoveryAdapter::liveIdentity)
        .collect(Collectors.toList());
  }

  @Override
  public List<RecoveryMetadata.LiveIdentity> listLiveByIds(List<Long> ids) {
    return ViewMetaService.getInstance().listLiveViewsByIds(ids).stream()
        .map(ViewRecoveryAdapter::liveIdentity)
        .collect(Collectors.toList());
  }

  @Override
  public long id(ViewEntity entity) {
    return entity.id();
  }

  @Override
  public ViewEntity loadLive(NameIdentifier identifier) {
    return ViewMetaService.getInstance().getViewByIdentifier(identifier);
  }

  @Override
  public ViewEntity restoreAtomically(
    ...
    return ViewMetaService.getInstance()
        .restoreView(identifier, deletion, restoredAt, acceptedEtag, effectiveExpiresAt);
  }
  ...
  public void invalidate(NameIdentifier identifier) {
    ...
    try {
      ...
      LOG.warn(
          "View {} was restored, but its local entity cache could not be invalidated",
          identifier,
          e);
    }
    for (SupportsRelationOperations.Type relationType : SupportsRelationOperations.Type.values()) {
      try {
        ...
        LOG.warn(
            "View {} was restored, but its local {} relation cache could not be invalidated",
            identifier,
            relationType,
            e);
      }
  ...
  private static RecoveryMetadata.DeletedSnapshot deletedSnapshot(ViewPO view) {
    return new RecoveryMetadata.DeletedSnapshot(
        view.getViewId(),
        view.getViewName(),
        new RecoveryMetadata.ParentIdentity(
            view.getMetalakeId(), view.getCatalogId(), view.getSchemaId()),
        view.getDeletedAt(),
        view.getCurrentVersion());
  }

  private static RecoveryMetadata.LiveIdentity liveIdentity(ViewPO view) {
    return new RecoveryMetadata.LiveIdentity(
        view.getViewId(), view.getSchemaId(), view.getViewName());
  }
}
