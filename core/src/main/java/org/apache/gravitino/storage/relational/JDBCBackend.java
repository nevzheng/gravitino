...
public class JDBCBackend implements RelationalBackend {
    ...
    switch (entityType) {
      case METALAKE:
        return MetalakeMetaService.getInstance()
            .deleteMetalake(ident, cascade, deletionRetentionMs);
      case CATALOG:
  ...
  public int hardDeleteLegacyData(Entity.EntityType entityType, long legacyTimeline)
    ...
    switch (entityType) {
      case METALAKE:
        int recoverableMetalakeDeletionCount =
            MetalakeMetaService.getInstance()
                .purgeExpiredMetalakeDeletions(
                    legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
        if (recoverableMetalakeDeletionCount > 0) {
          return recoverableMetalakeDeletionCount;
        }
        int legacyMetalakeCount =
            MetalakeMetaService.getInstance()
                .deleteMetalakeMetasByLegacyTimeline(
                    legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
        if (legacyMetalakeCount > 0) {
          return legacyMetalakeCount;
        }
      ...
      case SCHEMA:
        int recoverableSchemaDeletionCount =
            SchemaMetaService.getInstance()
                .purgeExpiredSchemaDeletions(
                    legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
        if (recoverableSchemaDeletionCount > 0) {
          return recoverableSchemaDeletionCount;
        }
        int legacySchemaCount =
            SchemaMetaService.getInstance()
                .deleteSchemaMetasByLegacyTimeline(
                    legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
        if (legacySchemaCount > 0) {
          return legacySchemaCount;
        }
        return EntityDeletionService.getInstance()
            .deleteTerminalReceipts(
                Entity.EntityType.SCHEMA, legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
      case TABLE:
      ...
      case FILESET:
        int recoverableFilesetDeletionCount =
            FilesetMetaService.getInstance()
                .purgeExpiredFilesetDeletions(
                    legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
        if (recoverableFilesetDeletionCount > 0) {
          return recoverableFilesetDeletionCount;
        }
        int legacyFilesetCount =
            FilesetMetaService.getInstance()
                .deleteFilesetAndVersionMetasByLegacyTimeline(
                    legacyTimeline, GARBAGE_COLLECTOR_SINGLE_DELETION_LIMIT);
        if (legacyFilesetCount > 0) {
          return legacyFilesetCount;
        }
        return EntityDeletionService.getInstance()
            .deleteTerminalReceipts(
